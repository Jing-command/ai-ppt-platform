...
import { Plus, FileText, Loader2, AlertCircle, RefreshCw, Clock, Layers } from 'lucide-react';
import { OutlineResponse, OutlineStatus } from '@/types/outline';
import { getOutlines, deleteOutline } from '@/lib/api/outlines';
...
const statusMap: Record<OutlineStatus, { label: string; color: string }> = {
...
export default function OutlinesPage() {
  ...
  const fetchOutlines = async () => {
    ...
    try {
      const response = await getOutlines({ page: 1, pageSize: 100 });
  ...
  const handleDelete = async (outline: OutlineResponse) => {
    if (!confirm(`确定要删除大纲 "${outline.title}" 吗？此操作不可恢复。`)) {
      return;
    }

    setDeleteLoading(outline.id);

    try {
      await deleteOutline(outline.id);
      setOutlines((prev) => prev.filter((o) => o.id !== outline.id));
    } catch (err) {
      const axiosError = err as AxiosError;
      if (axiosError.response?.status === 401) {
        router.push('/login');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-semibold text-[var(--color-text)]">大纲管理</h1>
            </div>

            <motion.button
              onClick={() => router.push('/outlines/new')}
            ...
            {outlines.length === 0 && !error ? (
              ...
              >
                <div
                  ...
                  "
                >
                  <FileText className="w-10 h-10 text-[var(--color-text-placeholder)]" />
                </div>
                <h3 className="text-lg font-medium text-[var(--color-text)]">
                  暂无大纲
                </h3>
                <p className="mt-2 text-[var(--color-text-muted)] max-w-md mx-auto">
                  还没有创建任何 PPT 大纲。使用 AI 生成你的第一个大纲吧。
                </p>
                <motion.button
                  onClick={() => router.push('/outlines/new')}
                  whileHover={{ scale: 1.02 }}
                    ...
                    >
                      ...
                      <div className="p-5 border-b border-[var(--color-border)]">
                        <div className="flex items-start justify-between">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusMap[outline.status].color}`}>
                            {statusMap[outline.status].label}
                          </span>
                        </div>
                        <p className="mt-2 text-sm text-[var(--color-text-muted)] line-clamp-2">
                          {outline.description || '暂无描述'}
                        </p>
                      </div>
